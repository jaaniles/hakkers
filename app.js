...
var baseMultiplierPerClick = 0.13;
var overloadPerClick = 0.08;
var overloadDecay = 0.2;

var baseBotnetIncome = 6;
...
var initialLotteryAmount = 200000;
var lotteryLength = 30 // in seconds
var lotteryMultiplier = 1.6

var minimumScoreForLoan = 50;
var baseLoanMultiplier = 5;
var baseLoansharkTime = 15;
...
setInterval(function(){
    ...
    if (lottery.timer == 0){
        ...
        if (lottery.attendees.length > 0){
            var winnerIndex = randomInt(0, (lottery.attendees.length - 1));
            ...
            var winAmount = Math.floor( lottery.jackpot * lottery.multiplier ).toLocaleString()
            var message = "You won " + winAmount + " crd in the lottery!";
            broadcastToConsole(message, winner);
        }
        // No attendees, reset lottery
        lottery.attendees = [];
        lottery.jackpot = initialLotteryAmount;
        lottery.timer = lotteryLength;
        lottery.multiplier = lotteryMultiplier;
    }
}, 1000)
...
createPlayer("1", "Anonymous") 
createPlayer("2", "Anonymous") 
createPlayer("3", "Anonymous") 
createPlayer("4", "Anonymous") 
createPlayer("5", "Anonymous") 
createPlayer("6", "Anonymous") 
createPlayer("7", "Anonymous") 
createPlayer("8", "Anonymous") 
createPlayer("9", "Anonymous") 
createPlayer("10", "Anonymous") 
createPlayer("11", "Anonymous") 
createPlayer("12", "Anonymous") 
createPlayer("13", "Anonymous") 
createPlayer("14", "Anonymous") 
createPlayer("15", "Anonymous") 
createPlayer("16", "Anonymous") 
createPlayer("17", "Anonymous") 
...
io.on('connection', function(socket) {
    ...
    socket.on('sendCurrency', function(data) {
        ...
        if (data.license == null || amount <= 0 || !amount) {
            return;
        }
        var player = players[findWithCode(players, code)];
    ...
    socket.on("loan", function(data) {
        ...
        if (player.purchasedApps["loanshark"]) {
            ...
            if (loanshark.takenLoan > 0) {
                ...
                if (loanshark.paybackTime <= 1 || !paybackTimer){
                    player.paybackTimer = setInterval(function() {
                        ...
                        if (loanshark.paybackTime < 1) {
                            ...
                            broadcastToConsole(message,
                                player);
                            clearInterval(player.paybackTimer);
                            player.paybackTimer = null;
                        }
                    }, 1000)
                }
            }
